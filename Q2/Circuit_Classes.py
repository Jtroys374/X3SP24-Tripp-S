...
class Position():
    ...
    def getStr(self, nPlaces=3, SI=True):
        lenCF = 1 if SI else 3.3
        return '{}, {}, {}'.format(round(self.x * lenCF, nPlaces), round(self.y * lenCF, nPlaces),
    ...
    def midPt(self, p2=None):
        ...
        :param p2: a position        :return:  in middle between self and p2
        """
...
class circuitView():
    ...
    def drawCircuit(self, Model=circuitModel()):
        for n in Model.nodes:
            if n.draw:
                self.drawACircle(centerX=n.position.x * self.scale, centerY=n.position.y * self.scale,
                                 Radius=1 * self.scale, brush=self.brushNode, pen=self.penNode, tooltip=n.name)
    ...
    def drawVoltageSource(self, V=voltageSource(), Model=circuitModel()):
        ...
        # Draw +/- symbols
        self.drawALine(StartPt=Position(x=center.x - radius / 3, y=center.y - radius / 3),
                       EndPt=Position(x=center.x + radius / 3, y=center.y - radius / 3), Pen=self.penConnect)
        self.drawALine(StartPt=Position(x=center.x - radius / 3, y=center.y + radius / 3),
                       EndPt=Position(x=center.x + radius / 3, y=center.y + radius / 3), Pen=self.penConnect)
        self.drawALine(StartPt=Position(x=center.x, y=center.y),
                       EndPt=Position(x=center.x, y=center.y + radius / 3 + radius / 3), Pen=self.penConnect)

        self.drawALine(StartPt=n1.position * self.scale,
                       EndPt=self.scale * (n1.position + (1 - self.elementFraction) / 2 * vec), Pen=self.penConnect)
        self.drawALine(StartPt=n2.position * self.scale,
                       EndPt=self.scale * (n2.position - (1 - self.elementFraction) / 2 * vec), Pen=self.penConnect)
    ...
    def drawResistor(self, R=resistor(), Model=circuitModel()):
        ...
        vec = n2.position - n1.position
        # Step 3: determine orientation and containing rectangle of the resistor
        ...
        # Step 4: draw the resistor symbol
        # region draw the resistor symbol
        painter = QtGui.QPainter(self.scene())
        painter.setRenderHint(QtGui.QPainter.Antialiasing)
        pen = QtGui.QPen(qtc.Qt.black, 2)
        painter.setPen(pen)
        if not vert:
            # Horizontal resistor
            startX = center.x - width / 2
            startY = center.y
            path = QtGui.QPainterPath(qtc.QPointF(startX, startY))
            for i in range(5):
                xOffset = (i + 1) * width / 5
                yOffset = height if i % 2 == 0 else -height
                path.lineTo(qtc.QPointF(startX + xOffset, startY + yOffset))
        else:
            # Vertical resistor
            startX = center.x
            ...
            path = QtGui.QPainterPath(qtc.QPointF(startX, startY))
            for i in range(5):
                xOffset = width if i % 2 == 0 else -width
                yOffset = (i + 1) * height / 5
                path.lineTo(qtc.QPointF(startX + xOffset, startY + yOffset))
        painter.drawPath(path)
        painter.end()
        # endregion
        # region connect resistor to nodes
        self.drawALine(StartPt=n1.position * self.scale,
                       EndPt=self.scale * (n1.position + (1 - self.elementFraction) / 2 * vec), Pen=self.penConnect)
        self.drawALine(StartPt=n2.position * self.scale,
                       EndPt=self.scale * (n2.position - (1 - self.elementFraction) / 2 * vec), Pen=self.penConnect)
        ...
        if not vert:
            ...
            for i in range(4):
                if i == 0:
                    ...
                    stAng = 300
                    spnAng = 300
                self.drawAnArc(centerX=(left + (i + 1) * width / 5), centerY=center.y, Radius=width / 5,
                               startAngle=stAng, spanAngle=spnAng, pen=self.penConnect)
        else:
            ...
            for i in range(4):
                if i == 0:
                    ...
                    stAng = 300 - 90
                    spnAng = 300
                self.drawAnArc(centerX=center.x, centerY=(bottom + (i + 1) * (height / 5)), Radius=height / 5,
                               startAngle=stAng, spanAngle=spnAng, pen=self.penConnect)
        # endregion
    ...
    def drawCapacitor(self, C=capacitor(), Model=circuitModel()):
        ...
        if not vert:
            ...
            self.drawALine(StartPt=Position(x=center.x - gap, y=bottom), EndPt=Position(x=center.x - gap, y=top),
                           Pen=self.penConnect)
            self.drawALine(StartPt=Position(x=center.x + gap, y=bottom), EndPt=Position(x=center.x + gap, y=top),
                           Pen=self.penConnect)
        else:
            ...
            self.drawALine(StartPt=Position(x=left, y=center.y - gap), EndPt=Position(x=right, y=center.y - gap),
                           Pen=self.penConnect)
            self.drawALine(StartPt=Position(x=left, y=center.y + gap), EndPt=Position(x=right, y=center.y + gap),
                           Pen=self.penConnect)
