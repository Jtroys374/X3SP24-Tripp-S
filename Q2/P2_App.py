    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.Controller.View.scene:
            ...
            if event.type() == qtc.QEvent.GraphicsSceneMouseMove:
                ...
                s = self.Controller.View.scene.itemAt(scenePos,
                                                      self.gv_Main.transform())  # gets item from graphics scene under the mouse
                strScene = "Mouse Position:  x = {}, y = {}".format(round(scenePos.x(), 2),
                                                                    round(-scenePos.y(), 2))  # $NEW$ 4/7/21 flip y
                if s is not None and s.data(
                        0) is not None:  # when creating nodes and pipes, I used the setData() function to store a name
                    strScene += ' ' + s.data(0)
